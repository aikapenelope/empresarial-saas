import type {
  CollectionAfterChangeHook,
  CollectionAfterDeleteHook,
  CollectionBeforeDeleteHook,
  CollectionBeforeValidateHook,
  CollectionConfig,
} from 'payload';
import {
  applySupplierPaymentAllocations,
  extractId,
  recalculateSupplierBalance,
  reverseSupplierPaymentAllocations,
  type SupplierPaymentAllocation,
} from '../../utilities/purchasesLedger';

const beforeValidateSupplierPayment: CollectionBeforeValidateHook = async ({
  data,
  req,
}) => {
  if (!data) return data;

  // 1. Calculate multicurrency totals
  if (Array.isArray(data.methods)) {
    let sumTotalUSD = 0;
    data.methods = data.methods.map((methodItem) => {
      const amount = Number(methodItem.amount) || 0;
      const rate = Number(methodItem.exchangeRate) || 1;
      let amountUSD = amount;

      if (methodItem.currency === 'VES') {
        amountUSD = rate > 0 ? Number((amount / rate).toFixed(2)) : 0;
      } else {
        amountUSD = Number(amount.toFixed(2));
      }

      sumTotalUSD += amountUSD;
      return {
        ...methodItem,
        amountUSD,
      };
    });

    data.totalUSD = Number(sumTotalUSD.toFixed(2));
  }

  // 2. Financial invariant validations on allocations
  if (Array.isArray(data.allocations) && data.allocations.length > 0) {
    const supplierId = extractId(data.supplier);
    let totalAllocated = 0;

    for (const alloc of data.allocations) {
      const allocatedAmount = Number(alloc.allocatedAmountUSD) || 0;
      if (allocatedAmount <= 0) {
        throw new Error('El monto asignado a cada factura de compra debe ser mayor a cero.');
      }
      totalAllocated += allocatedAmount;

      const invoiceId = extractId(alloc.purchaseInvoice);
      if (invoiceId) {
        const invoice = await req.payload.findByID({
          collection: 'purchase-invoices',
          id: invoiceId,
          depth: 0,
          req,
          context: {
            ...req.context,
            skipBalanceRecalculation: true,
          },
        });

        if (!invoice) {
          throw new Error(
            `La factura de compra con ID ${invoiceId} asignada en el egreso no existe.`,
          );
        }

        if (supplierId && extractId(invoice.supplier) !== supplierId) {
          throw new Error(
            `La factura ${invoice.invoiceNumber || invoiceId} pertenece a otro proveedor y no puede ser imputada en este egreso.`,
          );
        }

        if (invoice.status === 'voided') {
          throw new Error(
            `La factura ${invoice.invoiceNumber || invoiceId} está anulada y no puede recibir pagos.`,
          );
        }

        if (invoice.status === 'draft') {
          throw new Error(
            `La factura ${invoice.invoiceNumber || invoiceId} es un borrador y no puede recibir pagos.`,
          );
        }

        const paymentTenantId = extractId(data.tenant);
        const invoiceTenantId = extractId(invoice.tenant);
        if (
          paymentTenantId &&
          invoiceTenantId &&
          String(paymentTenantId) !== String(invoiceTenantId)
        ) {
          throw new Error(
            `La factura ${invoice.invoiceNumber || invoiceId} pertenece a otro inquilino y no puede ser abonada en este egreso.`,
          );
        }
      }
    }

    if (totalAllocated > (Number(data.totalUSD) || 0) + 0.01) {
      throw new Error(
        `El total asignado a facturas ($${totalAllocated.toFixed(2)} USD) no puede exceder el monto total del egreso ($${(Number(data.totalUSD) || 0).toFixed(2)} USD).`,
      );
    }
  }

  // Cash Register and Shift Session validations
  const tenantId = extractId(data.tenant);
  const registerId = extractId(data.cashRegister);
  let closureId = extractId(data.cashClosure);

  if (registerId && !closureId) {
    const openShift = await req.payload.find({
      collection: 'cash-closures',
      where: {
        and: [
          { cashRegister: { equals: registerId } },
          { status: { equals: 'open' } },
        ],
      },
      limit: 1,
      depth: 0,
      req,
    });
    if (openShift.totalDocs > 0) {
      closureId = openShift.docs[0].id;
      data.cashClosure = closureId;
    }
  }

  if (closureId) {
    const closure = await req.payload.findByID({
      collection: 'cash-closures',
      id: closureId,
      depth: 0,
      req,
    });
    if (!closure) {
      throw new Error(`El turno de caja con ID ${closureId} no existe.`);
    }
    if (closure.status !== 'open') {
      throw new Error(
        `El turno de caja ${closure.closureNumber || closureId} ya está cerrado o auditado. No se pueden registrar egresos en un turno cerrado.`,
      );
    }
    const closureTenantId = extractId(closure.tenant);
    if (tenantId && closureTenantId && String(tenantId) !== String(closureTenantId)) {
      throw new Error('El turno de caja seleccionado pertenece a otra empresa.');
    }
  }

  return data;
};

const afterChangeSupplierPayment: CollectionAfterChangeHook = async ({
  doc,
  previousDoc,
  req,
}) => {
  if (req.context?.skipBalanceRecalculation) return doc;

  const currentSupplierId = extractId(doc.supplier);
  const currentTenantId = extractId(doc.tenant);
  const previousSupplierId = extractId(previousDoc?.supplier);
  const previousTenantId = extractId(previousDoc?.tenant);

  // If previous doc was confirmed, reverse its allocations first
  if (previousDoc?.status === 'confirmed' && Array.isArray(previousDoc.allocations)) {
    await reverseSupplierPaymentAllocations(
      previousDoc.allocations as SupplierPaymentAllocation[],
      req,
      {
        supplierId: previousSupplierId,
        tenantId: previousTenantId,
      },
    );
  }

  // If current doc is confirmed, apply its allocations
  if (doc.status === 'confirmed' && Array.isArray(doc.allocations)) {
    await applySupplierPaymentAllocations(
      doc.allocations as SupplierPaymentAllocation[],
      req,
      {
        supplierId: currentSupplierId,
        tenantId: currentTenantId,
      },
    );
  }

  if (currentSupplierId) {
    await recalculateSupplierBalance(currentSupplierId, req);
  }

  if (previousSupplierId && previousSupplierId !== currentSupplierId) {
    await recalculateSupplierBalance(previousSupplierId, req);
  }

  return doc;
};

const beforeDeleteSupplierPayment: CollectionBeforeDeleteHook = async ({
  id: _id,
  req,
}) => {
  const doc = await req.payload.findByID({
    collection: 'supplier-payments',
    id: _id,
    depth: 0,
    req,
    context: {
      ...req.context,
      skipBalanceRecalculation: true,
    },
  });

  if (doc?.status === 'confirmed' && Array.isArray(doc.allocations)) {
    await reverseSupplierPaymentAllocations(
      doc.allocations as SupplierPaymentAllocation[],
      req,
      {
        supplierId: extractId(doc.supplier),
        tenantId: extractId(doc.tenant),
      },
    );
  }
};

const afterDeleteSupplierPayment: CollectionAfterDeleteHook = async ({
  doc,
  req,
}) => {
  if (req.context?.skipBalanceRecalculation) return doc;

  const supplierId = extractId(doc?.supplier);
  if (supplierId) {
    await recalculateSupplierBalance(supplierId, req);
  }

  return doc;
};

export const SupplierPayments: CollectionConfig = {
  slug: 'supplier-payments',
  labels: {
    singular: 'Pago a Proveedor / Egreso',
    plural: 'Pagos a Proveedores / Egresos',
  },
  admin: {
    useAsTitle: 'paymentNumber',
    group: 'Proveedores & CxP',
    defaultColumns: ['paymentNumber', 'supplier', 'totalUSD', 'paymentDate', 'status'],
  },
  access: {
    read: ({ req: { user } }) => Boolean(user),
    create: ({ req: { user } }) =>
      Boolean(
        user?.role === 'super-admin' ||
          user?.role === 'tenant-admin' ||
          user?.role === 'supervisor',
      ),
    update: ({ req: { user } }) =>
      Boolean(
        user?.role === 'super-admin' ||
          user?.role === 'tenant-admin' ||
          user?.role === 'supervisor',
      ),
    delete: ({ req: { user } }) =>
      Boolean(user?.role === 'super-admin' || user?.role === 'tenant-admin'),
  },
  hooks: {
    beforeValidate: [beforeValidateSupplierPayment],
    afterChange: [afterChangeSupplierPayment],
    beforeDelete: [beforeDeleteSupplierPayment],
    afterDelete: [afterDeleteSupplierPayment],
  },
  fields: [
    {
      name: 'paymentNumber',
      label: 'Número de Comprobante de Egreso / Pago',
      type: 'text',
      required: true,
      index: true,
    },
    {
      name: 'supplier',
      label: 'Proveedor',
      type: 'relationship',
      relationTo: 'suppliers',
      required: true,
      index: true,
    },
    {
      name: 'paymentDate',
      label: 'Fecha del Egreso / Pago',
      type: 'date',
      required: true,
      defaultValue: () => new Date().toISOString(),
      admin: {
        date: {
          pickerAppearance: 'dayAndTime',
        },
      },
    },
    {
      name: 'status',
      label: 'Estado del Pago',
      type: 'select',
      required: true,
      defaultValue: 'confirmed',
      options: [
        { label: 'Por Confirmar / En Tránsito', value: 'pending' },
        { label: 'Confirmado / Aplicado', value: 'confirmed' },
        { label: 'Rechazado / Reversado', value: 'rejected' },
      ],
    },
    {
      name: 'methods',
      label: 'Desglose Multimoneda / Métodos de Egreso',
      type: 'array',
      required: true,
      minRows: 1,
      fields: [
        {
          name: 'method',
          label: 'Método / Canal de Salida',
          type: 'select',
          required: true,
          options: [
            { label: 'Efectivo Dólares (USD)', value: 'cash_usd' },
            { label: 'Efectivo Bolívares (VES)', value: 'cash_ves' },
            { label: 'Punto de Venta / Tarjeta (VES)', value: 'pos_ves' },
            { label: 'Zelle (USD)', value: 'zelle' },
            { label: 'Pago Móvil (VES)', value: 'pago_movil' },
            { label: 'Transferencia Bancaria (VES)', value: 'transfer_ves' },
            { label: 'Binance Pay / USDT', value: 'binance' },
          ],
        },
        {
          name: 'currency',
          label: 'Moneda Egresada',
          type: 'select',
          required: true,
          defaultValue: 'USD',
          options: [
            { label: 'Dólares (USD)', value: 'USD' },
            { label: 'Bolívares (VES)', value: 'VES' },
          ],
        },
        {
          name: 'amount',
          label: 'Monto Egresado en la Moneda',
          type: 'number',
          required: true,
          min: 0.01,
        },
        {
          name: 'exchangeRate',
          label: 'Tasa de Conversión aplicada a USD',
          type: 'number',
          required: true,
          defaultValue: 1,
          min: 0.0001,
        },
        {
          name: 'amountUSD',
          label: 'Equivalente Neto en USD',
          type: 'number',
          admin: {
            readOnly: true,
          },
        },
        {
          name: 'reference',
          label: 'Referencia / Confirmación Bancaria de Salida',
          type: 'text',
        },
        {
          name: 'receipt',
          label: 'Comprobante Digital Adjunto',
          type: 'upload',
          relationTo: 'media',
        },
      ],
    },
    {
      name: 'totalUSD',
      label: 'Monto Total Egresado (USD)',
      type: 'number',
      required: true,
      min: 0.01,
      admin: {
        readOnly: true,
      },
    },
    {
      name: 'allocations',
      label: 'Asignación a Facturas de Compra (CxP)',
      type: 'array',
      fields: [
        {
          name: 'purchaseInvoice',
          label: 'Factura de Compra a Abonar',
          type: 'relationship',
          relationTo: 'purchase-invoices',
          required: true,
        },
        {
          name: 'allocatedAmountUSD',
          label: 'Monto Abonado a la Factura (USD)',
          type: 'number',
          required: true,
          min: 0.01,
        },
      ],
    },
    {
      name: 'cashRegister',
      label: 'Caja Registradora (Punto de Egreso)',
      type: 'relationship',
      relationTo: 'cash-registers',
      index: true,
      admin: {
        position: 'sidebar',
      },
    },
    {
      name: 'cashClosure',
      label: 'Turno de Caja / Cierre',
      type: 'relationship',
      relationTo: 'cash-closures',
      index: true,
      admin: {
        position: 'sidebar',
      },
    },
    {
      name: 'notes',
      label: 'Notas del Egreso',
      type: 'textarea',
    },
  ],
  timestamps: true,
};
